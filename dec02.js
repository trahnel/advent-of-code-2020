...
// Part 1
const part1answer = input.reduce((aggr, string) => {
  const firstSpaceIndex = string.indexOf(' ');
  const [charMin, charMax] = string.substr(0, firstSpaceIndex).split('-');

  const colonIndex = string.indexOf(':');
  const passwordChar = string[colonIndex - 1];

  const password = string.substr(colonIndex + 2);

  const charCount = Array.from(password).reduce(
    (aggr, char) => (char === passwordChar ? aggr + 1 : aggr),
    0
  );

  // console.log({ charMin, charMax, passwordChar, password, charCount });
  return charCount >= charMin && charCount <= charMax ? aggr + 1 : aggr;
}, 0);
...
// Part 2
// '4-5 t: ftttttrvts',
const part2answer = input.reduce((aggr, string) => {
  const firstSpaceIndex = string.indexOf(' ');
  const [pos1, pos2] = string.substr(0, firstSpaceIndex).split('-');
  ...
  const pos1Valid = password[pos1 - 1] === passwordChar;
  const pos2Valid = password[pos2 - 1] === passwordChar;

  console.log({ pos1, pos2, passwordChar, password, pos1Valid, pos2Valid });
  return pos1Valid !== pos2Valid ? aggr + 1 : aggr;
}, 0);
